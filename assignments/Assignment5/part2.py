...
def detect_shape(contour, circle_threshold=0.8) -> Shape:
    ...
    return ("Circle", Shape.CIRCLE) if 4 * np.pi * a / p**2 > circle_threshold else ("Square", Shape.SQUARE)
...
def process_image():
    ...
    img = cv2.cvtColor(
        cv2.imread("raw_img.png"), cv2.COLOR_BGR2RGB
    )
    ...
    for i in range(len(filtered)):
        ...
        for j in range(len(filtered[i])):
            ...
            cv2.circle(all_contours_img, get_center(filtered[i][j]), 5, (255, 255, 0), -1)
            features.append({
                ...
                "orientation": get_orientation(filtered[i][j], detect_shape(filtered[i][j]))
            })
            # label each contour below center
            cv2.putText(all_contours_img, f"Shape: {features[-1]['shape']} Size: {features[-1]['size']}", (get_center(filtered[i][j])[0] - 100, get_center(filtered[i][j])[1] - 50), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1, cv2.LINE_AA)
            cv2.putText(all_contours_img, f"WorldPos: ({round(features[-1]['pos'][0],2)}, {round(features[-1]['pos'][1],2)}) Center: {features[-1]['center']}", (get_center(filtered[i][j])[0] - 100, get_center(filtered[i][j])[1] - 90), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1, cv2.LINE_AA)
            cv2.putText(all_contours_img, f"Color: {features[-1]['color']} Angle: {features[-1]['orientation']}", (get_center(filtered[i][j])[0] - 100, get_center(filtered[i][j])[1] - 70), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1, cv2.LINE_AA)
