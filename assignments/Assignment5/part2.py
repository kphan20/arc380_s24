...
def process_image():
    ...
    for i in range(len(filtered)):
        ...
        for j in range(len(filtered[i])):
            ...
            cv2.putText(all_contours_img, f"{features[-1]['shape']} {features[-1]['size']} {round(features[-1]['pos'][0],2)} {round(features[-1]['pos'][1],2)}", (get_center(filtered[i][j])[0] - 100, get_center(filtered[i][j])[1] - 50), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
            cv2.putText(all_contours_img, f"{features[-1]['center']} {features[-1]['color']} {features[-1]['orientation']}", (get_center(filtered[i][j])[0] - 100, get_center(filtered[i][j])[1] - 70), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
